...
#define ENCODER_PIN1 PINB1
#define ENCODER_PIN2 PINB0

#define ENCODER2_PORT PORTC
#define ENCODER2_DDR DDRC
#define ENCODER2_PIN PINC

#define ENCODER2_PIN1 PINC0
#define ENCODER2_PIN2 PINC1

static unsigned char _state;
static unsigned char _eventIsUsed;
static unsigned char _rotaryEncoder_direction;

static unsigned char _state2;
...
void rotaryEncoder_process() {
	...
	if ((ENCODER2_PIN & (1<<ENCODER2_PIN1))!= 0)
	{
		pinstate2 |= (1 << 0);
	}
	if ((ENCODER2_PIN & (1<<ENCODER2_PIN2))!= 0)
	{
		pinstate2 |= (1 << 1);
	}
	
	// Determine new state from the pins and state table.
    _state2 = ttable[_state2 & 0xf][pinstate2];
	// Return emit bits, ie the generated event.
	if(_eventIsUsed || ((_state2 & 0x30) != DIR_NONE) ) 
	{
		_rotaryEncoder_direction = (_state2 & 0x30) + 0x01;
		_eventIsUsed = 0;
	}
}
...
ENCODER_SPIN_DIRECTION rotaryEncoder_get_direction()
{
	...
	switch(_rotaryEncoder_direction)
	{
        ...
        case DIR2_CCW:
			return ENCODER2_SPIN_DIRECTION_LEFT;
		case DIR2_CW:
			return ENCODER2_SPIN_DIRECTION_RIGHT;
		default:
